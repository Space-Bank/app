...
export const Stake = () => {
  ...
  const loadInventory = async () => {
    try {
      ...
      setGsmBalance(Number(ethers.utils.formatEther(BigNumber.from(currentGsm))))

      let stakedMobBosses = await stakingContract.getMobBossStakedNFTTokens(web3.account)
      setTotalStaked(stakedMobBosses.length)

      let data: any = [];
      for (var i = 0; i < totalStaked; i++) {
        data.push(Number(ethers.utils.formatEther(BigNumber.from(stakedMobBosses[i]))) * 10 ** 18);
      }
      setStakedNfts(data)

      let totalRewardAmount = await stakingContract.getMobBossRewardTotalAmount(web3.account)
      setClaimableGsm(Number(ethers.utils.formatEther(BigNumber.from(totalRewardAmount))))
      ...
      <div className="z-10 flex flex-col pl-12 sm:pl-36 md:pl-48 w-full text-left space-y-4 pb-12">
        ...
        <div className='text-white flex flex-wrap space-x-20 space-y-4 justify-start items-center w-full pb-4'>
          ...
          <div>
            ...
            <p className='text-lg font-saira-sb text-primary'><span className='text-white'>$</span>GSM</p>
          </div>
        </div>
      </div>
      ...
      <div className='z-40 space-y-12 mx-6 flex-col flex items-center'>
        ...
        <div className="w-full max-w-2xl flex items-center justify-between rounded-sm bg-gradient-to-tr to-white/20 from-white/40 ring-1 ring-white/40 shadow-md p-2">
          ...
          <button onClick={(e: any) => {
            e.preventDefault()
            claimGsmAll()
          }} className='flex py-2 px-3 bg-primary hover:scale-105 duration-300 transition-all ease-in-out'>CLAIM ALL</button>
        </div>
            ...
            <div className="justify-center w-full  items-center mx-1.5 flex flex-col space-y-4">
              ...
              <div className="flex items-center w-full justify-between">
                ...
                <div className="text-white">
                  {20 * totalStaked} <span className='text-lg font-saira-sb text-primary'><span className='text-white'>$</span>GSM</span> PER DAY
                </div>
              </div>
                    ...
                    >
                      <div className="">
                        <span className="flex space-x-1 items-center px-1 pb-1.5">
                          <span className='font-saira-m text-2xl mr-1.5'>MOB BOSS</span> {i}
                        </span>
                        <Image
                      ...
                      <button onClick={(e: any) => {
                        e.preventDefault()
                        unstakeNft(i)
                      }} className='flex py-2 px-3 bg-primary hover:scale-105 duration-300 transition-all ease-in-out'>UNSTAKE</button>

                      <button onClick={(e: any) => {
                        e.preventDefault()
                        claimGsmNft(i)
                      }} className='flex py-2 px-3 bg-primary hover:scale-105 duration-300 transition-all ease-in-out'>CLAIM $GSM</button>
                    </div>))
                  }
                </div>
              </div>
